...
features = ['apache_3j_diagnosis','gcs_motor_apache', 'd1_lactate_max', 'd1_lactate_min','apache_4a_hospital_death_prob', 'apache_4a_icu_death_prob','gcs-eyes-apache']
...
def main():
    with st.form('prediction_form'):
        ...
        gcs_eyes_apache = st.selectbox('gcs-eyes-apache:', [1.0, 2.0, 3.0, 3.4650492139135083, 4.0])
        submit = st.form_submit_button("Predict")

    if submit:

        data= np.array([apache_3j_diagnosis, gcs_motor_apache, d1_lactate_max, d1_lactate_min, apache_4a_hospital_death_prob, apache_4a_icu_death_prob, gcs_eyes_apache]).reshape(1, -1)
